...
/*--------------------------------------------------------------------------*/
/**
 * 处理html字符串中的标签和实体（entity）
 * @param {String} contents
 * @returns {String}
 */
export const getHtmlText = (contents) => {
  let text = contents;
  if (!text) {
    return text;
  }
  try {
    const HE_DECODE_CONFIG = {
      isAttributeValue: false, // used in a text context in an HTML document.
      strict: false, // don't throw error even if it contains any entities that cause parse errors
    };
    text = contents.replace(/(<[^>]+>)/gi, '');
    text = decode(text, HE_DECODE_CONFIG);
  } catch (e) {
    console.log(e);
  }
  return text;
