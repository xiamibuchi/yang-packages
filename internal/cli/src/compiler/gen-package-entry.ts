...
import { SRC_DIR, getPackageJson, getSyConfig } from '../common/constant.js';
...
export function genPackageEntry({
  ...
  const namedExport = syConfig.build?.namedExport || false;
  const skipInstall = (syConfig.build?.skipInstall || []).map(pascalize);
